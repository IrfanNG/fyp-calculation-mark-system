...
export default function SubmitReportPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(`Report submitted successfully! Version ${data.submission.version}`);
      setFile(null);
  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div>
        <h1 className="text-xl font-semibold">Submit FYP Report</h1>
        <p className="text-sm text-(--unikl-muted)">Upload your FYP report for supervisor review. Accepted: PDF, DOCX, DOC.</p>
      </div>
      ...
      <div className="rounded-lg border border-(--unikl-border) bg-(--unikl-card) p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="rounded-md bg-(--unikl-bg) p-3 text-xs text-(--unikl-muted) space-y-1">
            ...
            <p>• Each submission creates a new version — you can re-submit after rejection</p>
          </div>
